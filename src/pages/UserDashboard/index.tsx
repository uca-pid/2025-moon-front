...
function getVehicleColor(vehiclePlate: string): string {
  if (!VEHICLE_COLORS[vehiclePlate]) {
    const colorIndex = Object.keys(VEHICLE_COLORS).length % COLOR_PALETTE.length
          ...
          {payload.map((p: any) => (
            <div key={p.dataKey} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-foreground font-medium">
                {mode === "count" ? `${p.value} veces` : `$${Number(p.value).toLocaleString()}`}
              </span>
            </div>
          ))}
        </div>
    ...
    <Container>
      <div className="max-w-7xl space-y-6">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Análisis de servicios y estadísticas de tus vehículos</p>
        </div>

        <div className="bg-card rounded-3xl p-6 shadow-sm border border-border/50">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              <h2 className="text-xl font-semibold text-foreground mb-1">Estadísticas de Servicios</h2>
              <p className="text-sm text-muted-foreground">Comparación por vehículo y tipo de servicio</p>
            </div>
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="h-[400px] animate-in fade-in slide-in-from-bottom-4 duration-700">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData} margin={{ left: 10, right: 10, top: 20, bottom: 60 }} barCategoryGap="20%">
            ...
            <div className="h-[400px] flex flex-col items-center justify-center text-center">
              ...
              <p className="text-lg font-medium text-foreground mb-2">No hay datos disponibles</p>
              <p className="text-sm text-muted-foreground">Los datos aparecerán cuando haya servicios registrados</p>
            </div>
          )}

          <div className="flex items-center gap-2 mt-6 pt-6 border-t border-border/50">
            <TrendingUp className="h-4 w-4 text-primary" />
            <span className="text-sm text-muted-foreground">Análisis histórico de servicios realizados</span>
          </div>
        </div>
        ...
        <div className="bg-card rounded-3xl p-6 shadow-sm border border-border/50">
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-foreground mb-1">Historial de Servicios</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-foreground text-sm">{h.workshop.workshopName}</p>
                      </div>
                    </div>
