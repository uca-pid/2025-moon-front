...
export function Reserve() {
  ...
  const handleDisabled = () => {
    return !selectedServices.length || !date || !time || selectedServices.length === 0 || selectedVehicle === ""
  }
          ...
          <div className="space-y-6">
            <div className="group bg-card rounded-3xl p-8 shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <Label className="text-lg font-semibold text-foreground">Fecha y Horario</Label>
              </div>
              <DatePicker
                hasTimePicker={true}
                date={date}
            ...
            <div className="group bg-card rounded-3xl p-8 shadow-sm hover:shadow-md transition-all duration-300">
              ...
              {vehicles.length > 0 ? (
                <Select value={selectedVehicle} onValueChange={(value) => setSelectedVehicle(value)}>
                  ...
                  <SelectContent className="rounded-xl">
                    {vehicles.map((vehicle) => (
                      <SelectItem key={vehicle.id} value={vehicle.id.toString()} className="rounded-lg">
                ...
                <div className="p-4 rounded-xl bg-gray-50 border border-gray-100">
                  <p className="text-sm text-gray-500">No tienes ningún vehículo registrado.</p>
                </div>
              )}
            </div>
            ...
            <div className="group bg-card rounded-3xl p-8 shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <Label className="text-lg font-semibold text-foreground">Taller</Label>
              </div>
              <div className="flex gap-3 items-center">
                ...
                <Dialog open={showMap} onOpenChange={setShowMap}>
                  ...
                  <DialogContent className="max-w-4xl h-[60vh] flex flex-col rounded-3xl">
                    <DialogHeader>
                      <DialogTitle className="text-2xl font-bold">Ubicaciones de talleres</DialogTitle>
                    </DialogHeader>
            ...
            <div className="group bg-card rounded-3xl p-8 shadow-sm hover:shadow-md transition-all duration-300">
              <div className="flex items-center gap-3 mb-6">
                ...
                <Label className="text-lg font-semibold text-foreground">Servicios</Label>
              </div>
              {!workshop ? (
                <div className="p-4 rounded-xl bg-card border border-gray-100">
                  <p className="text-sm text-gray-500">Seleccione un taller primero.</p>
                </div>
              ) : services.length > 0 ? (
                <MultiSelect
                ...
                <div className="p-4 rounded-xl bg-card border border-gray-100">
                  <p className="text-sm text-gray-500">Este taller no tiene servicios disponibles por el momento.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:sticky lg:top-8 h-fit">
            <div className="bg-card rounded-3xl p-8 shadow-sm text-foreground">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-xl font-bold text-foreground">Resumen del turno</h3>
              </div>
                ...
                <div className="space-y-4 text-foreground">
                  {date && (
                    <div className="flex items-start gap-4 p-4 rounded-2xl bg-card shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-foreground mb-1">Fecha</p>
                  ...
                  {time && (
                    <div className="flex items-start gap-4 p-4 rounded-2xl bg-card shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-foreground mb-1">Horario</p>
                  ...
                  {selectedVehicle && (
                    <div className="flex items-start gap-4 p-4 rounded-2xl bg-card shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-foreground mb-1">Vehículo</p>
                        <p className="text-sm font-semibold text-foreground">
                          {vehicles.find((v) => v.id.toString() === selectedVehicle)?.licensePlate}
                        </p>
                        <p className="text-xs text-foreground">
                          {vehicles.find((v) => v.id.toString() === selectedVehicle)?.model}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {workshop && (
                    <div className="flex items-start gap-4 p-4 rounded-2xl bg-card shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-foreground mb-1">Taller</p>
                        <p className="text-sm font-semibold text-foreground">
                  ...
                  {selectedServices.length > 0 && (
                    <div className="flex items-start gap-4 p-4 rounded-2xl bg-card shadow-sm">
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-foreground mb-1">Servicios</p>
                        <div className="space-y-1">
                          {selectedServices.map((id) => {
                            const service = services.find((s) => s.id.toString() === id)
                            return (
                              <p key={id} className="text-sm font-semibold text-foreground">
                                {service?.name}
                                {service?.price && (
                                  <span className="text-foreground font-normal"> - ${service.price}</span>
                                )}
                              </p>
                            )
                          })}
